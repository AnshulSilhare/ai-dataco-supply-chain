...
# ──────────────────────────────────────────────
# 3. GLOBAL CSS — cyberpunk logistics terminal
#    Uses explicit wrapper divs, no brittle
#    Streamlit-internal selectors.
# ──────────────────────────────────────────────
st.markdown("""
<style>
/* ── Fonts ── */
...
/* Animated scanline + noise background */
.stApp {
    background:
...
.glass:hover {
    ...
    box-shadow:
        ...
        0 0 40px -10px rgba(0,229,255,0.12);
}
...
/* Slider */
.stSlider > div[data-baseweb] > div > div {
...
.stButton > button:hover {
    color: #fff !important;
    border-color: var(--cyan) !important;
    box-shadow: 0 0 32px rgba(0,229,255,0.35), inset 0 0 24px rgba(0,229,255,0.1) !important;
...
.terminal {
    ...
    overflow: hidden;
    box-shadow:
        0 0 40px rgba(0,255,163,0.08) inset,
        0 0 0 1px rgba(0,255,163,0.06);
    position: relative;
}
.terminal::before {
    content: '● NEXUS NEURAL CORE  ○ READY';
    position: absolute;
...
.result-success::after {
    ...
    background: linear-gradient(120deg,
        transparent 0%,
        rgba(0,255,163,0.1) 40%,
        transparent 80%);
    background-size: 200% auto;
    animation: shine-success 3s linear infinite;
}
...
.gauge-track { fill:none; stroke:rgba(255,255,255,0.05); stroke-width:10; stroke-linecap:round; }
.gauge-fill  { fill:none; stroke-width:10; stroke-linecap:round; stroke-dasharray:157; stroke-dashoffset:157;
               animation: fill-gauge 1.4s cubic-bezier(.2,.8,.2,1) forwards; }
@keyframes fill-gauge { to { stroke-dashoffset: var(--offset); } }
.gauge-center {
...
.feat-fill {
    ...
    animation: grow-bar 1.1s cubic-bezier(.1,.6,.2,1) forwards;
}
...
[data-testid="stDownloadButton"] > button:hover {
    ...
    box-shadow: 0 0 20px rgba(0,255,163,0.25) !important;
}
...
[data-testid="stExpander"] {
    border: 1px solid var(--border) !important;
    border-radius: 12px !important;
    background: rgba(0,5,15,0.5) !important;
}
...
# ──────────────────────────────────────────────
# 4. PARTICLE CANVAS (behind everything)
# ──────────────────────────────────────────────
components.html("""
...
(function(){
  ...
  const N_PARTICLES = 55, N_NODES = 12, LINK_DIST = 160;
  function resize(){ W=canvas.width=window.innerWidth; H=canvas.height=window.innerHeight; }
  window.addEventListener('resize',resize); resize();

  for(let i=0;i<N_PARTICLES;i++) particles.push({
    x:Math.random()*W, y:Math.random()*H,
    vx:(Math.random()-.5)*.3, vy:(Math.random()-.5)*.3,
    r: Math.random()*1.5+.5, a: Math.random()
  });
  for(let i=0;i<N_NODES;i++) nodes.push({
    x:Math.random()*W, y:Math.random()*H,
  ...
  function draw(){
    ...
    for(let i=0;i<nodes.length;i++){
      ...
      for(let j=i+1;j<nodes.length;j++){
        let m=nodes[j], d=Math.hypot(n.x-m.x,n.y-m.y);
        if(d<LINK_DIST){
          ctx.save(); ctx.globalAlpha=(1-d/LINK_DIST)*0.3;
    ...
    for(let p of particles){
      ...
      ctx.save(); ctx.globalAlpha=p.a*0.4;
...
<script>
  ...
  if(el){
    const msg = "Real-time delivery risk intelligence · Route deviation analysis · Fleet optimization";
...
# ──────────────────────────────────────────────
# 6. KPI RIBBON (native HTML counter)
# ──────────────────────────────────────────────
components.html("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Space+Mono:wght@700&family=Syne:wght@700&display=swap');
...
<div class="kb">
  <div class="kc">
    ...
    <div class="kv"><span class="cnt" data-t="73.31" data-d="1">0.0</span><span class="ku">%</span></div>
  </div>
  <div class="kc">
    <div class="kl">Routes Monitored</div>
    <div class="kv"><span class="cnt" data-t="180519" data-d="0">0</span></div>
  </div>
  <div class="kc">
    <div class="kl">Avg Inference Time</div>
    <div class="kv"><span class="cnt" data-t="0.4" data-d="1">0.0</span><span class="ku">s</span></div>
  </div>
</div>
<script>
document.querySelectorAll('.cnt').forEach(el=>{
  const target=+el.dataset.t, dec=+el.dataset.d;
  const dur=1800, fps=60, steps=dur/(1000/fps);
  let cur=0, step=0;
  const id=setInterval(()=>{
    step++;
    cur=target*(step/steps);
    if(step>=steps){ cur=target; clearInterval(id); }
...
with tab_single:
    ...
    with col_r:
        days_scheduled = st.number_input(
            "Scheduled Transit Days", min_value=0, max_value=10, value=3,
            help="SLA-agreed delivery window"
        )
        order_item_quantity = st.slider(
            "Freight Quantity (Units)", 1, 5, 1
        )
    ...
    if run_btn:
        ...
        logs = [
            ...
            ("Computing SHAP feature attribution",             "ok"),
        ...
        try:
            ...
            data_dict: dict[str, int] = {col: 0 for col in expected_cols}
            ...
            with r1:
                ...
                # Gauge — half-circle SVG rendered via components.html
                # so CSS custom properties and JS animations work correctly
                dash_total  = 157          # π × r=50 (half-circle arc length)
                dash_offset_end = dash_total - (dash_total * probability)
                gauge_color = (
                    "#f43f5e" if probability > 0.65 else
                    "#f59e0b" if probability > 0.35 else
                    "#00ffa3"
                )
                components.html(f"""
                <style>
                  ...
                  .g-fill {{
                    ...
                    stroke-dashoffset: {dash_total};      /* start fully empty */
                    filter: drop-shadow(0 0 6px {gauge_color});
                    transition: stroke-dashoffset 1.4s cubic-bezier(.2,.8,.2,1);
                  }}
                  /* Label sits at the visual centre of the arc.
                     The arc centre is the SVG point (60, 65).
                     viewBox=0 0 120 80 → the SVG is 120 wide, 80 tall.
                     (65/80)=81.25% down from the top of the SVG.
                     The wrapper div is sized to the SVG height via padding-bottom trick.
                     We use absolute positioning inside. */
                  .g-label {{
                    position: absolute;
                    /* arc centre = cx 60, cy 65 inside viewBox 0 0 120 80          */
                    /* left = 60/120 = 50 %;  top = 65/80 = 81.25 %                 */
                    left: 50%;
                    top: 81.25%;
                    transform: translate(-50%, -50%);
                    text-align: center;
                  ...
                  .g-pct {{
                    ...
                    font-weight: 700;
                    color: {gauge_color};
                    text-shadow: 0 0 18px {gauge_color}80;
                    line-height: 1;
                    display: block;
                  }}
                  .g-sub {{
                ...
                <div class="g-wrap">
                  <!--
                    viewBox 0 0 120 80:
                      Arc centre = (60, 65), radius = 50
                      Left tip  = (10, 65), Right tip = (110, 65)
                    Extra 10px of height (70→80) gives breathing room below arc.
                  -->
                ...
                <script>
                  // Trigger CSS transition after a short delay so browser paints first
                  setTimeout(function(){{
            ...
            # RIGHT — feature importance + radar + timeline
            with r2:
                st.markdown('<div class="glass">', unsafe_allow_html=True)
                ...
                for _, row in imp_df.iterrows():
                    ...
                    <div class="feat-row">
                      ...
                      <div class="feat-track">
                        <div class="feat-fill" style="--w:{w:.1f}%"></div>
                      </div>
                      <div class="feat-pct">{pct_label}</div>
                    </div>"""
                bars_html += '</div>'
                ...
                fig_tl = px.bar(
                    tl_df, x="Finish", y="Task", color="Type", orientation='h',
                ...
                fig_tl.update_layout(
                    ...
                    xaxis={'title': "Days After Dispatch", 'showgrid': True,
                           'gridcolor': 'rgba(255,255,255,0.06)', 'color': '#64748b'},
...
with tab_bulk:
    ...
    st.markdown("""
    <p style="font-family:var(--font-mono);font-size:.8rem;color:var(--muted);margin-bottom:1.5rem;line-height:1.7">
    Upload a CSV with columns: <code>Shipping Mode</code>, <code>Order Region</code>,
    <code>Days_Scheduled</code>, <code>Order_Item_Quantity</code>, <code>Sales</code>,
    <code>Order_Profit_Per_Order</code>
    </p>
    """, unsafe_allow_html=True)
    ...
    if uploaded:
        try:
            ...
            st.markdown(f'<div class="sec-sub">Preview — {len(bulk_df):,} records detected</div>',
                        unsafe_allow_html=True)
            st.dataframe(bulk_df.head(8), use_container_width=True)

            if st.button("⬡  EXECUTE ENTERPRISE BATCH PREDICTION"):
                with st.spinner("⬡  Nexus Engine classifying batch..."):
                    expected_cols = model.feature_names_in_
                    processed = []
                    for _, row in bulk_df.iterrows():
                        d: dict[str, float] = {col: 0 for col in expected_cols}
                        if "Days_Scheduled"        in expected_cols: d["Days_Scheduled"]        = row.get("Days_Scheduled", 3)
                        if "Order_Item_Quantity"    in expected_cols: d["Order_Item_Quantity"]    = row.get("Order_Item_Quantity", 1)
                        if "Sales"                 in expected_cols: d["Sales"]                 = row.get("Sales", 150.0)
                        if "Order_Profit_Per_Order" in expected_cols: d["Order_Profit_Per_Order"] = row.get("Order_Profit_Per_Order", 20.0)
                ...
                s1.metric("At-Risk Shipments", f"{n_at_risk:,}", delta=f"{n_at_risk/len(preds)*100:.1f}% of batch", delta_color="inverse")
                s2.metric("On-Time Predicted", f"{n_on_time:,}", delta=f"{n_on_time/len(preds)*100:.1f}% of batch")
                ...
                st.download_button(
                    ...
                    data=csv_out,
                    file_name="nexus_batch_predictions.csv",
                    mime="text/csv",
